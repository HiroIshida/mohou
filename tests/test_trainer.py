...
def test_traincache_load_all(tmp_project_name):  # noqa
    ...
    for _ in range(10):
        dump_train_cache(conf, np.random.rand(), get_project_path(tmp_project_name))

    conf2 = LSTMConfig(3, 3, 3, 2)  # whatever
    for _ in range(10):
        dump_train_cache(conf2, np.random.rand(), get_project_path(tmp_project_name))

    assert len(TrainCache.load_all(get_project_path(tmp_project_name))) == 20
    assert len(TrainCache.load_all(get_project_path(tmp_project_name), LSTM)) == 20
    assert len(TrainCache.load_all(get_project_path(tmp_project_name), LSTM, conf)) == 10
    assert len(TrainCache.load_all(get_project_path(tmp_project_name), LSTM, conf2)) == 10
...
def test_traincache_load(tmp_project_name):  # noqa
    ...
    # test loading
    TrainCache.load(get_project_path(tmp_project_name), LSTM)
    TrainCache.load(get_project_path(tmp_project_name), LSTM, conf)

    with pytest.raises(FileNotFoundError):
        wrong_conf = LSTMConfig(6, 6, 666, 2)
        TrainCache.load(get_project_path(tmp_project_name), LSTM, wrong_conf)
...
def test_traincache_load_best_one(tmp_project_name):  # noqa
    ...
    for loss_value in np.linspace(3, 10, 5):
        dump_train_cache(conf, loss_value, get_project_path(tmp_project_name))

    tcache = TrainCache.load(get_project_path(tmp_project_name), LSTM)
