...
class LSTMConfig(ModelConfigBase):
    ...
    n_bias: int = 0  # type independent bias input
    n_hidden: int = 200
    n_layer: int = 4
...
class LSTM(ModelBase[LSTMConfig]):
    ...
    lstm with bias: x_t+1 = f(x_t, x_t-1, ..., b) where b is not time dependent
    """
    ...
    def _setup_from_config(self, config: LSTMConfig) -> None:
        ...
        n_bias = config.n_bias
        self.lstm_layer = nn.LSTM(
            n_state + n_bias, config.n_hidden, config.n_layer, batch_first=True
        )
        output_layers = []
    ...
    def loss(self, sample: Tuple[torch.Tensor, torch.Tensor, torch.Tensor]) -> LossDict:
        state_sample, bias_sample, weight_seqs = sample

        # sanity check
        n_batch, n_seq_len, n_dim = state_sample.shape
        assert bias_sample.ndim == 2
        assert bias_sample.shape[0] == n_batch
        assert bias_sample.shape[1] == self.config.n_bias
        ...
        pred_output = self.forward(state_sample_input, bias_sample)
    ...
    def forward(self, state_sample: torch.Tensor, bias_sample: torch.Tensor) -> torch.Tensor:
        # arrange bais_sample and create concat state_sample
        _, n_seq_len, _ = state_sample.shape
        bias_unsqueezed = bias_sample.unsqueeze(dim=1)
        bias_sequenced = bias_unsqueezed.expand(-1, n_seq_len, -1)
        bias_auged_state_sample = torch.cat((state_sample, bias_sequenced), dim=2)

        # propagation
        tmp, _ = self.lstm_layer(bias_auged_state_sample)
